...
class SumoIntersection:
    ...
    def inference_with_tflite(self, tflite_model_path):
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and stepz < 7000:
            ...
            # Prepare input for TF Lite model
            input_state = [state[0].astype(np.float32), state[1].astype(np.float32), state[2].astype(np.float32)]
            interpreter.set_tensor(input_details[0]['index'], input_state)

            # Perform inference
            interpreter.invoke()

            # Get the output
            output_action = interpreter.get_tensor(output_details[0]['index'])
...
if __name__ == '__main__':
    ...
    '''try:
      ...
      try:
	    ...
	    interpreter = tf.lite.Interpreter(model_path='/home/arsh/BTEP/DITLACS_Git/CodeBase/7_TF_Lite_Inference/keras_model.tflite')
	    interpreter.allocate_tensors()
	    input_details = interpreter.get_input_details()
	    output_details = interpreter.get_output_details()
